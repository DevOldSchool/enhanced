...
public class NpcInspectServiceTest
{
	...
	@Test
	public void resolvesEncodedRelativeRedirectAndCachesSuccessfulInspect() throws Exception
	{
		QueuedResponseInterceptor responses = new QueuedResponseInterceptor();
		responses.enqueueRedirect("/w/Goblin%20guard#Level_2");
		...
		assertEquals("Goblin guard", first.getWikiPage());
		...
		assertEquals("npc", responses.requests().get(0).url().queryParameter("type"));
		assertEquals("3028", responses.requests().get(0).url().queryParameter("id"));
		assertEquals("Goblin guard", responses.requests().get(0).url().queryParameter("name"));
		...
		assertEquals("Goblin guard", responses.requests().get(1).url().queryParameter("page"));
	}
