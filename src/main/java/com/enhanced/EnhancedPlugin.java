...
import com.enhanced.item.ItemPriceSummary;
import com.enhanced.item.ItemRequirementSummary;
import com.enhanced.item.ItemInspectService;
import com.enhanced.inspect.InspectPanel;
import com.enhanced.npc.BankEquipmentOverlay;
import com.enhanced.npc.BankEquipmentRecommendationService;
import com.enhanced.npc.EquipmentRecommendation;
import com.enhanced.npc.NpcCombatInfo;
import com.enhanced.npc.NpcInspectService;
import com.enhanced.player.PlayerEquipmentComparison;
import com.enhanced.player.PlayerEquipmentItem;
import com.enhanced.player.PlayerInspectAnalysis;
import com.enhanced.inspect.RecentInspectEntry;
...
@PluginDescriptor(
	name = "Enhanced",
	description = "Enhances OSRS gameplay with optional information tools.",
	tags = {"enhanced", "npc", "item", "equipment", "inspect", "wiki"}
)
public class EnhancedPlugin extends Plugin
{
	private static final String CONFIG_GROUP = "enhanced";
	...
	protected void startUp()
	{
		...
		inspectPanel.setCacheManagementHandler(new InspectPanel.CacheManagementHandler()
		{
			...
			public void clearAllCache()
			{
				itemInspectService.clearCacheAsync();
				npcInspectService.clearCacheAsync();
				SwingUtilities.invokeLater(() -> inspectPanel.showSearchDisabled("Enhanced Inspect caches cleared."));
			}
		});
	...
	private void inspectPlayerEquipmentItem(int itemId, String itemName)
	{
		...
		{
			SwingUtilities.invokeLater(() -> inspectPanel.showSearchDisabled("Enable OSRS Wiki lookups in the Enhanced config to inspect items from player equipment."));
			return;
		}

		if (!config.showItemInspectOption())
		{
			SwingUtilities.invokeLater(() -> inspectPanel.showSearchDisabled("Enable Item inspect in the Enhanced config to inspect items from player equipment."));
			return;
		}
	...
	private void searchInspect(String type, String query)
	{
		...
		if (!config.enableWikiLookups())
		{
			SwingUtilities.invokeLater(() -> inspectPanel.showSearchDisabled("Enable OSRS Wiki lookups in the Enhanced config to search the OSRS Wiki from this panel."));
			return;
		}

		if (!config.showInspectSearch())
		{
			SwingUtilities.invokeLater(() -> inspectPanel.showSearchDisabled("Enable Inspect search in the Enhanced config to search from this panel."));
			return;
		}
	...
	@Provides
	EnhancedConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(EnhancedConfig.class);
	}
